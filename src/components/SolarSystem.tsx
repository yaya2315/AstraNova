...
const Sun = memo(function Sun() {
  ...
  const texture = useMemo(() => {
    const t = new THREE.TextureLoader().load('/textures/8k_sun.jpg')
...
function Planet({ data, speedMul, onHover, onLeave, onClick }: {
  ...
  const texture = useMemo(() => {
    const t = new THREE.TextureLoader().load(data.textureUrl)
    t.colorSpace = THREE.SRGBColorSpace
    return t
  }, [data.textureUrl])
...
const SaturnRings = memo(function SaturnRings({ size }: { size: number }) {
  ...
  const ringTex = useMemo(() => new THREE.TextureLoader().load('/textures/8k_saturn_ring_alpha.jpg'), [])
